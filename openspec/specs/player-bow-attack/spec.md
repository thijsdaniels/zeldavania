...
The player system SHALL support quick-firing and hold-to-aim shooting mechanics via the item action input.

#### Scenario: Quick tap fires immediately
- **WHEN** the player presses and releases the item button in less than the aim threshold (0.15s) while standing, running, or falling
...
- **WHEN** the player holds the item button for longer than the aim threshold (0.15s)
...
- **WHEN** the player releases the item button while in the aiming state
