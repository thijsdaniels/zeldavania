...
## Purpose
Implements readable environmental signposts throughout the world and a bottom-anchored in-game dialogue banner with typewriter text streaming, multi-page pagination, fast-forward skip, and dismiss capabilities.
...
### Requirement: Dialogue Banner Presentation & Multi-Page Pagination
...
- **THEN** all remaining characters on the current page are revealed immediately.

#### Scenario: Advancing multi-page dialogues
- **GIVEN** a dialogue message exceeds 3 lines or contains explicit page break delimiters
- **WHEN** the current page finishes revealing
- **THEN** the prompt displays `(A) Continue`
- **WHEN** the player presses Button A
- **THEN** the dialogue box flips to the next page and streams the next text segment.

#### Scenario: Dismissing dialogue and restoring control
- **GIVEN** the final page of dialogue text has completed its reveal
- **WHEN** the player presses the confirm action (Button A)
